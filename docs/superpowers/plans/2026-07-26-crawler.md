...
```bash
cd /Users/driversti/Projects/erepublik/babel
...
```bash
DATABASE_URL=postgresql://babel:babel@172.17.0.1:5432/babel
HOME_COUNTRY=XX
...
services:
  ...
  db:
    ...
    environment:
      - POSTGRES_USER=babel
      - POSTGRES_PASSWORD=${POSTGRES_PASSWORD:-babel}
      - POSTGRES_DB=babel
    volumes:
...
def test_collects_image_references_in_order():
    article = parse_article(load("article_with_images.html"), 2796980)
    assert len(article.images) >= 1
    assert [i.position for i in article.images] == list(range(len(article.images)))
...
- [ ] **Step 5: Rename fixtures to the names the tests expect**

The probe saved files as `article_{id}.html`. Pick one long non-English article and one image-heavy one:

```bash
cd /Users/driversti/Projects/erepublik/babel
uv run babel probe --newest 2797100 --count 1 --save-fixtures 0  # sanity check the CLI still runs
mv tests/fixtures/article_2797015.html tests/fixtures/article_indonesia.html
mv tests/fixtures/article_2796980.html tests/fixtures/article_with_images.html
```

If those exact IDs were not among the saved fixtures, fetch them directly:

```bash
for id in 2797015 2796980; do
  docker compose run --rm crawler \
    curl -s "https://www.erepublik.com/en/article/$id/1/1000" > "tests/fixtures/article_$id.html"
done
```
...
Run: `uv run pytest tests/crawler/test_parser.py -v`
Expected: 10 passed (5 parametrised eday cases + 5 others)
...
def test_depth_comes_from_indentation():
    ...
    assert max(c.depth for c in comments) >= 1
...
def test_parse_article_attaches_comments():
    article = parse_article(load("article_with_images.html"), 2796980)
    assert len(article.comments) == len(parse_comments(load("article_with_images.html")))
```
...
async def test_ingests_article_comments_and_images(pg, settings):
    ...
    assert await ingestor.ingest(2796980) == "ok"

    assert await pg.fetchval("SELECT count(*) FROM articles WHERE id = 2796980") == 1
    assert await pg.fetchval("SELECT count(*) FROM comments WHERE article_id = 2796980") > 0
    assert await pg.fetchval("SELECT status FROM fetch_log WHERE article_id = 2796980") == "ok"
    statuses = await pg.fetch("SELECT DISTINCT status FROM article_images WHERE article_id = 2796980")
...
async def test_dead_images_do_not_fail_the_article(pg, settings):
    ...
    assert await pg.fetchval(
        "SELECT count(*) FROM article_images WHERE article_id = 2796980 AND status = 'dead'"
    ) > 0
