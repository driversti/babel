...
async def test_ingests_article_comments_and_images(pg, fake_pool, settings):
    ...
    ingestor = Ingestor(fake_pool(pg), get_page, get_bytes, RateLimiter(1000), settings)
...
async def test_dead_images_do_not_fail_the_article(pg, fake_pool, settings):
    async def get_page(url):
        return 200, ARTICLE_HTML
...
async def test_text_is_saved_even_when_the_disk_is_full(pg, fake_pool, tmp_path):
    ...
    ingestor = Ingestor(fake_pool(pg), get_page, get_bytes, RateLimiter(1000), full)
    assert await ingestor.ingest(2797005) == "ok"
    assert await pg.fetchval("SELECT count(*) FROM articles WHERE id = 2797005") == 1
    assert await pg.fetchval(
...
async def test_image_capture_exception_mid_loop_does_not_abort_the_article(pg, fake_pool, settings, monkeypatch):
...
async def test_first_image_failure_does_not_block_the_second(pg, fake_pool, settings, monkeypatch):
    calls: list[str] = []
